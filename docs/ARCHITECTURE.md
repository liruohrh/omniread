...
│                                                                 │
│  数据流：                                                        │
│  HTML string → scraper::Html → JS 脚本查询/提取 → JSON → Dart    │
└─────────────────────────────────────────────────────────────────┘
```
...
| 函数 | 签名 | 说明 |
|------|------|------|
| `document.querySelector` | `(selector) → Element \| null` | CSS 选择器查询单个元素 |
| `document.querySelectorAll` | `(selector) → Element[]` | CSS 选择器查询所有匹配元素 |
| `getText` | `(element) → string` | 获取元素的文本内容 |
| `getAttr` | `(element, name) → string \| null` | 获取元素属性值 |
| `hasClass` | `(element, className) → boolean` | 检查元素是否有指定 class |
| `getInnerHtml` | `(element) → string` | 获取元素的 innerHTML |
| `selectWithin` | `(parent, selector) → Element \| null` | 在父元素内查询 |
| `selectAllWithin` | `(parent, selector) → Element[]` | 在父元素内查询所有 |
| `console.log` | `(...args) → void` | 调试输出 |
...
| `decode.base64(str)` | Base64 解码 |
| `decode.url(str)` | URL 解码 |
